...
#ifdef _WIN32
// POSIX setenv/unsetenv are absent from the MSVC/Windows CRT; shim them onto
// _putenv_s. On Windows an empty value string removes the variable.
#include <stdlib.h>
static inline int setenv(const char* name, const char* value, int overwrite)
{
    if (!overwrite) {
        size_t sz = 0;
        if (getenv_s(&sz, nullptr, 0, name) == 0 && sz != 0) return 0;
    }
...
int main(int argc, char* argv[])
{
    ...
    //
    // Windows has no SIGUSR1, so std::signal(SIGUSR1, ...) would not compile.
    // We disable signal-based tip-preempt on Windows and advertise
    // preempt:false, so the Python wrapper never sends SIGUSR1 (it gates on
    // this flag). The solver then just runs each slice to completion
    // (max_seconds) — a few wasted seconds on a mid-slice tip change, but
    // fully functional.
#ifndef _WIN32
...
#ifdef _WIN32
    std::cerr << "{\"event\":\"daemon_ready\",\"preempt\":false}" << std::endl;
#else
